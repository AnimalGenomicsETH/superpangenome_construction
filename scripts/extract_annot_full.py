...
with open(sys.argv[1]) as infile:
    ...
    for line in infile:
        if not line.startswith("#"):
            ...
            symbol_id = [x if x else "n" for x in symbol_id]
            gene_id = [annot[x+3] for x in ind_vartype]
            #gene_id = set([x if x else "n" for x in gene_id])
            gene_id = [x if x else "n" for x in gene_id]
            alt_allele = [annot[x-1] for x in ind_vartype]
            vareff = [annot[x] for x in ind_vartype]
            for ind,comp in enumerate(all_effect):
                print(token[0],token[1],len(alt),suppvec,len(ref),len(alt_allele[ind]),comp,symbol_id[ind],gene_id[ind],vareff[ind])
                # print(len(ind_vartype),all_effect,[len(x) for x in alt])
