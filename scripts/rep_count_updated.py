...
rule all:
    input:
        ...
        # expand(outgraph_dir + "/{prog}_graph/{prog}_{chromo}.og",chromo=range(25,26),prog=prog_list),
        # expand("vnsplit/vntr_coord_{chromo}_{breed}_{prog}.tsv",chromo=range(25,26),breed=breed_list,prog=prog_list),
        # expand("vnsplit/vntr_comb_{chromo}_{prog}.tsv",chromo=range(25,26),prog=prog_list),
        # expand("vnsplit/{chromo}_{prog}_vnseq_count.tsv",chromo=range(25,26),prog=prog_list),
...
rule split_vntr:
    ...
    shell:
        ...
        awk -v chromo={wildcards.chromo} '$1 == chromo {{ print chromo"_UCD",$2,$3  }}' OFS="\\t" {input} > {output}
...
def extract_fasta_region(line,TR_length):
    sequence={}
    fastadir="/cluster/work/pausch/danang/psd/scratch/assembly"
    flank_factor=10
    offset=flank_factor*TR_length
    for comp in line:
        token=comp.split(",")
        if len(token) > 2:
...
def count_VNTRs(sequences,TR):
    ...
    #allowed_errors = math.floor(math.sqrt(len(TR)))
    scaling="linear"
    TR_divergence_limit=0.2
    allowed_errors = int(len(TR)*TR_divergence_limit) if scaling != 'log' else math.floor(math.sqrt(len(TR)))
    ...
    for asm, sequence in sequences.items():
        ...
        try:
            counts[asm] = len(regex.findall(f"(?bV1)({TR}){{2i+2d+1s<={allowed_errors}}}",sequence,concurrent=True,timeout=30))
        except TimeoutError:
            print('HIT AN ERROR')
            counts[asm] = -1   
    return counts
...
def processs_line(line):
    ...
    vnid=token[0]
    trseq=trcomb[vnid]
    combseq=extract_fasta_region(token[1:],len(trseq))
    ...
    return ','.join(map(str, [vnid,trseq] + stats))
    # return 
...
rule detect_pattern:
    ...
    run:
        with open(input.regfile) as infile, open(output[0],"w") as outfile:
            ...
            print("vnid", "vnseq",*breed_list,sep=",",file=outfile)
            #print('chr,start,end,TR,alignments,' + ','.join(breeds),file=outfile)
            with Pool(18) as p:
...
rule combine_vntr_info:
    input:
        expand("vnsplit/{chromo}_{{prog}}_vnseq_count.tsv",chromo=chromo_list)
    output:
