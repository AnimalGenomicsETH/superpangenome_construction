...
rule all:
    input: expand("graph/minigraph/graph_{chromo}.gfa", chromo=chromo_list),
           ...
           expand("graph/{prog}/graph_{prog}.vg",prog=prog_list),
...
rule convert_ppgb:
    ...
    output: 
        "graph/pggb_{chromo}/pggb_{chromo}.vg",
        "graph/pggb_{chromo}/pggb_{chromo}.xg",
        "graph/pggb_{chromo}/pggb_{chromo}.pg",
    threads: 10
    resources:
        mem_mb= 2000,
        walltime= "04:00"
    ...
    shell:
        """
        vg convert --threads {threads} -g {input} -v > {output[0]}

        vg convert --threads {threads} -g {input} -v > {output[1]}

        vg convert --threads {threads} -g {input} -p > {output[2]}

        """

rule combine_pggb:
    input:  expand("graph/pggb_{chromo}/pggb_{chromo}.pg",chromo=chromo_list)
    output: 
        full_graph="graph/pggb/graph_pggb.vg"
    threads: 10
    resources:
        mem_mb = 5000,
        walltime = "04:00"
...
rule chop_pggb:
    input:  "graph/pggb/graph_pggb.vg"
    output: 
    ...
    shell:
        ...
        vg mod --threads {threads} -X 256 {input} > {output.chop_graph}
